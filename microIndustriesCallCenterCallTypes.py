...
# Function to load JSON data from file - profile of each micro-industry
...
for idx, micro_industry_profile in enumerate(mi_profile_list, start=1):
    ...
    try:
        ...
        with open(text_output_path, "a") as text_file:
            text_file.write(f"Micro-industry {idx}:\n")
            text_file.write(json_output + "\n\n")
            print(f"Output for micro-industry {idx} saved to text.txt")
        #Saving each response in separate JSON file
